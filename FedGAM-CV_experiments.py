...
for ep in [15]:
    ...
    def get_args():
        ...
        parser.add_argument('--logdir', type=str, required=False, default="./FedGAM_SC_SVHN_logs_beta=0.3/", help='Log directory path')
        parser.add_argument('--modeldir', type=str, required=False, default="./models/", help='Model directory path')

        #for FedGAM
        parser.add_argument("--use_GAM", default=True, type=bool, help="True if you want to use GAM.")
        parser.add_argument("--base_opt", default='SGD', type=str, help="")  #args_use_SAM
    ...
    def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, device="cpu",args_use_GAM=False):
    ...
    #输出：训练后在训练数据集与测试数据集上的准确率
        ...
        if args_use_GAM == False:
            ...
            if not grad_rho_scheduler and not grad_norm_rho_scheduler and not optimizer:
                ...
                grad_norm_rho_scheduler = ProportionScheduler(pytorch_lr_scheduler=lr_scheduler, max_lr=args.lr,
                                                              min_lr=0.0,
                                                              max_value=args.grad_norm_rho_max,
                                                              min_value=args.grad_norm_rho_min)

                optimizer = GAM(params=net.parameters(), base_optimizer=base_optimizer, model=net,
            ...
            for epoch in range(epochs):
                ...
                for tmp in train_dataloader:
                    for batch_idx, (x, target) in enumerate(tmp):
                        ...
                        #在PyTorch中，long()函数用于将数据转换为长整型（Long）张量。
                        #在处理分类问题时，标签数据通常需要为长整型，因为损失函数（如交叉熵损失CrossEntropyLoss）需要接收长整型的标签数据。
                        optimizer.set_closure(loss_fn, x, target)
    ...
    #返回训练后各客户端的模型列表
        ...
        for net_id, net in nets.items():  #对每个被选中的客户端进行模型训练
            ...
            trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, device=device,args_use_GAM=args_use_GAM)  #训练
    ...
    def train_net_scaffold(net_id, net, global_model, c_local, c_global, train_dataloader, test_dataloader, epochs, lr, args_optimizer, device="cpu",args_use_GAM=False):
    ...
    def local_train_net_scaffold(nets, selected, global_model, c_nets, c_global, args, net_dataidx_map, test_dl = None, device="cpu",args_use_GAM=True):
        ...
        for net_id, net in nets.items():
            ...
            trainacc, testacc, c_delta_para = train_net_scaffold(net_id, net, global_model, c_nets[net_id], c_global, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, device=device,args_use_GAM=True)
    ...
    if __name__ == '__main__':
        ...
        if args.alg == 'fedavg':
            ...
            for round in range(args.comm_round):
                ...
                local_train_net_scaffold(nets, selected, global_model, c_nets, c_global, args, net_dataidx_map, test_dl = test_dl_global, device=device,args_use_GAM=args_use_GAM)
                # local_train_net(nets, args, net_dataidx_map, local_split=False, device=device)

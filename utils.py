...
#记录并返回每个参与方（网络节点）拥有的数据的类别统计信息。例如，查看哪些类别的数据在哪些参与方中比较多，哪些类别的数据在哪些参与方中比较少
    net_cls_counts = {}

    for net_i, dataidx in net_dataidx_map.items():
        unq, unq_cnt = np.unique(y_train[dataidx], return_counts=True)#统计每个参与方（网络节点）拥有的数据中各个类别的数量。
...
##返回训练数据（X_train, y_train）、测试数据（X_test, y_test）、各个客户端拥有的数据索引、各参与方（网络节点）拥有的数据的类别统计信息
    #np.random.seed(2020)
    ...
    #数据异构环境设置
    if partition == "homo":  #IID分布
        idxs = np.random.permutation(n_train)
        #创建一个长度为n_train的随机排列数组，n_train是训练数据的数量。这个数组包含了从0到n_train-1的所有整数，每个整数代表一个训练样本的索引。
        batch_idxs = np.array_split(idxs, n_parties)#将idxs数组均匀地划分为n_parties个部分
        net_dataidx_map = {i: batch_idxs[i] for i in range(n_parties)}


    elif partition == "noniid-labeldir":  #迪利克雷数据异构环境
        min_size = 0
        min_require_size = 10
        K = 10  #类别数
        ...
        while min_size < min_require_size:
            ...
            for k in range(K):
                ...
                #对每个类别 k，找出训练数据中所有该类别的数据索引 idx_k，然后随机打乱
                proportions = np.random.dirichlet(np.repeat(beta, n_parties))#使用狄利克雷分布（Dirichlet distribution）生成一个随机的比例 proportions，这个比例用于决定每个参与方获取该类别数据的比例
                # beta 的值较小，那么生成的比例可能会有较大的差异
                proportions = np.array([p * (len(idx_j) < N / n_parties) for p, idx_j in zip(proportions, idx_batch)])
                #确保每个参与方都能获取到足够的数据，避免某些参与方获取的数据过多，而其他参与方获取的数据过少
                # logger.info("proportions2: ", proportions)
                proportions = proportions / proportions.sum()
                # logger.info("proportions3: ", proportions)
                proportions = (np.cumsum(proportions) * len(idx_k)).astype(int)[:-1]#计算每个参与方获取数据的具体数量
                # logger.info("proportions4: ", proportions)
                idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k, proportions))]#将类别 k 的数据索引 idx_k 分配给各个参与方
                min_size = min([len(idx_j) for idx_j in idx_batch])#检查每个参与方的数据量是否满足最小要求 min_require_size。如果不满足，那么就重新进行分配
    ...
    elif partition > "noniid-#label0" and partition <= "noniid-#label9":  #客户端只拥有某几类数据
        ...
        if dataset == "cifar100":
            ...
            K = 200
        if num == 10:
            #如果 num 是 10，那么对于每个类别 i，找出训练数据中所有该类别的数据索引 idx_k，然后随机打乱，将 idx_k 均匀地划分为 n_parties 个部分
            ...
            times=[0 for i in range(K)]  #用于记录每个类别的数据应该被分配给多少个参与方
            contain=[] #用于记录每个参与方应该获取哪些类别的数据
            for i in range(n_parties):  #对于每个参与方 i，随机选择 num 个类别，将这些类别的数据分配给该参与方
                current=[i%K]
                times[i%K]+=1
                j=1
                while (j<num):
            ...
            net_dataidx_map ={i:np.ndarray(0,dtype=np.int64) for i in range(n_parties)}
            for i in range(K): #对于每个类别 i，找出训练数据中所有该类别的数据索引 idx_k，然后随机打乱，将 idx_k 划分为 times[i] 个部分，每个参与方 j 都会获取一个部分的数据索引，如果该类别在 contain[j]中
                idx_k = np.where(y_train==i)[0]
                np.random.shuffle(idx_k)
                split = np.array_split(idx_k,times[i])  #该类别数据均分给各个客户端
                ids=0
                for j in range(n_parties):
                    if i in contain[j]:
    ...
    elif partition == "iid-diff-quantity":  #各个客户端拥有的数据量不同（每方至少10个数据）
        idxs = np.random.permutation(n_train)
        min_size = 0
        while min_size < 10:
    ...
    elif partition == "mixed":#各个参与方拥有的数据既不同分布也不同数量（都通过迪利克雷操控）
        min_size = 0
        min_require_size = 10
        K = 10
        if dataset in ('celeba', 'covtype', 'a9a', 'rcv1', 'SUSY'):
...
#计算模型在给定的数据加载器（dataloader）上的准确率
    ...
    #在进行模型评估或测试时确保模型处于正确的模式。在完成评估或测试后，需要根据was_training的值将模型切换回原来的模式
        ...
        for tmp in dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                if moon_model:
                    ...
                    out = model(x)
                _, pred_label = torch.max(out.data, 1)  #获取模型输出的最大值对应的索引，即预测的类别标签。
    ...
    if get_confusion_matrix:  #如果get_confusion_matrix为True，还会计算混淆矩阵
